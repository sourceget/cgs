...
class AES {
     ...
     */
    public static function encrypt($contents, $encryptKey=null) {
        if(!$encryptKey){
            $encryptKey = env('APP_KEY');
        }
        // 将非标量数据转换为字符串
        ...
        // 进行AES/CBC加密
        $encryptedData = mcrypt_encrypt(MCRYPT_RIJNDAEL_256, $encryptKey, $contents, MCRYPT_MODE_CBC, $iv) . $iv;
     ...
     */
    public static function decrypt($contents, $encryptKey=null) {
        ...
        // 进行AES/CBC解密
        $contents = mcrypt_decrypt(MCRYPT_RIJNDAEL_256, $encryptKey, $contents, MCRYPT_MODE_CBC, $iv);
        // 获取Padding的值
